    ...
    return function (items) {
      ...
      var rows = items.map(function (it) {
        ...
        return '<div style="display:flex;gap:10px;align-items:center;justify-content:space-between">' +
          '<span>' + it.marker + escapeHtml(it.seriesName) + '</span>' +
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: function (items) {
          ...
          return '<div style="font-weight:600;margin-bottom:4px">' + escapeHtml(it.name) + "</div>" +
            '<div style="display:flex;gap:12px;justify-content:space-between">' +
            "<span>" + marker + escapeHtml(params.title) + "</span>" +
            '<strong style="font-variant-numeric:tabular-nums">' + escapeHtml(disp) + "</strong></div>";
        }
      },
      xAxis: {
        type: "category",
