...
Open `http://127.0.0.1:8766/`. English is the entry language. The nine pages have matching paths under `ko/`; the language switch retains the current page.

The build uses only Node's standard library. Content records are in `content.mjs`, current manuscripts in `current-research.mjs`, event records in `gallery-data.mjs`, HTML structure and bilingual page copy in `templates.mjs`, presentation in `site.css`, and browser behaviour in `site.js`. `telemetry-view.mjs` renders the static recorded plot; `telemetry.mjs` enhances it with playback. Running the build writes all 18 HTML pages. Keep the generated HTML with the source for direct local review.
