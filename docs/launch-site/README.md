...
Open `http://127.0.0.1:8767/`. English is the entry language. The eleven pages have matching paths under `ko/`; the language switch retains the current page.
...
`node check.cjs` expects the local server at port 8767. It uses the bundled Playwright installation and a fresh headless Edge process. Set `PSI_URL` to use another loopback origin. The checker closes its browser in a `finally` block and returns a non-zero exit status on a failed assertion.
