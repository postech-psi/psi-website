...
The checks cover all 16 pages at 320, 390, 768 and 1440 pixels; theme persistence; route-preserving language links; research topic/search filters, reset and empty state; mobile menu Escape/scroll restoration; selected hardware panels; intentional media switching; actual pad/onboard playback; font and image loading; and browser exceptions. The included homepage checks cover activity media/content/destination changes, tab associations, arrows/Home/End, playback control handoff and no-JavaScript fallbacks in both languages. Run `node check-homepage.cjs` for this focused subset. Desktop/mobile screenshots are written locally and excluded from version control. The root review scripts add independent all-theme/local-anchor checks.
...
Videos never autoplay. Visitors select the launch-pad or onboard view and explicitly play it. The onboard view has a rapid-camera-rotation warning. With JavaScript, one initial play overlay hands playback to native controls, which remain available on pause/end for resume, replay, seek, volume and fullscreen. Switching viewpoint resets the paused clip and its initial play affordance. Without JavaScript, native video controls remain available. Homepage activity tabs switch real imagery, explanatory copy and destinations; without JavaScript, all three activities are readable in sequence. Theme selection stores only `psi-theme` in local browser storage and follows the system setting when selected.
